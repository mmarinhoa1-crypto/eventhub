      ...
      {!temDados ? (
        <div className="bg-white rounded-xl border border-gray-200 p-8">
          <div className="max-w-lg mx-auto text-center">
            <div className="w-20 h-20 bg-blue-100 dark:bg-blue-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6"><Database className="text-blue-600 dark:text-blue-400" size={36} /></div>
            ...
            <div className="bg-blue-50 dark:bg-blue-500/10 rounded-xl p-5 text-left space-y-3 mb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
              <h2 className="font-semibold text-gray-900 flex items-center gap-2"><Calculator size={18} className="text-blue-600 dark:text-blue-400" />Parâmetros</h2>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Nome do Evento</label><input type="text" value={nomeEvento} onChange={e => setNomeEvento(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-accent focus:border-accent" placeholder="Ex: Show do Hungria" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Tipo de Local</label>
                <select value={tipo} onChange={e => setTipo(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-accent focus:border-accent">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Tipo de Bar</label>
                <select value={tipoBar} onChange={e => setTipoBar(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-accent focus:border-accent"><option value="bar_vendido">Bar Vendido</option><option value="open_bar">Open Bar</option><option value="misto">Misto</option></select></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Público Esperado</label><div className="relative"><Users size={16} className="absolute left-3 top-2.5 text-gray-400" /><input type="number" value={publico} onChange={e => setPublico(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-accent focus:border-accent" placeholder="Ex: 2000" /></div></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Cachê Artístico Total (R$)</label><div className="relative"><Music size={16} className="absolute left-3 top-2.5 text-gray-400" /><input type="number" value={cache} onChange={e => setCache(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-accent focus:border-accent" placeholder="Ex: 120000" /></div></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Ticket Médio Ingresso (R$)</label><div className="relative"><DollarSign size={16} className="absolute left-3 top-2.5 text-gray-400" /><input type="number" value={ticketMedio} onChange={e => setTicketMedio(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-accent focus:border-accent" placeholder="Ex: 80" /></div></div>
              <button onClick={calcular} className="w-full py-2.5 bg-accent text-white rounded-lg font-medium text-sm hover:bg-accent/90 transition-all flex items-center justify-center gap-2"><Sparkles size={16} />Gerar Previsão</button>
            </div>
            ...
            {resultado && resultado.refEventos.length > 0 && (<div className="bg-emerald-50 dark:bg-emerald-500/10 rounded-xl border border-emerald-200 dark:border-emerald-500/30 p-4"><h3 className="text-sm font-semibold text-emerald-900 dark:text-emerald-400 mb-2">Referência ({resultado.baseEventos} eventos)</h3>{resultado.refEventos.map((ref, i) => (<p key={i} className="text-xs text-emerald-700 dark:text-emerald-400">{ref}</p>))}</div>)}
          </div>

          <div className="lg:col-span-2 space-y-4">
            {!resultado ? (
              <div className="bg-white rounded-xl border border-gray-200 p-12 flex flex-col items-center justify-center text-center"><div className="w-16 h-16 bg-blue-100 dark:bg-blue-500/20 rounded-2xl flex items-center justify-center mb-4"><Sparkles className="text-blue-600 dark:text-blue-400" size={28} /></div><h3 className="text-lg font-semibold text-gray-900 mb-2">Previsão baseada nos seus eventos</h3><p className="text-sm text-gray-500 max-w-md">Preencha os parâmetros e clique em "Gerar Previsão".</p></div>
            ) : (
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`rounded-xl border p-4 ${resultado.lucroEstimado >= 0 ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/30' : 'bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/30'}`}><p className="text-xs text-gray-500 mb-1">Lucro Estimado</p><p className={`text-lg font-bold ${resultado.lucroEstimado >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}`}>{fmt(resultado.lucroEstimado)}</p><p className="text-xs text-gray-400">Ticket mín: {fmt(resultado.ticketMinimo)}</p></div>
                </div>
