...
export default function ConvidarModal({ open, onClose, onCreated }) {
  ...
  const [form, setForm] = useState({ nome: '', email: '', senha: '', funcao: opcoes[0]?.value || 'agent' })
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      await api.post('/equipe/convidar', form)
      toast.success('Membro convidado!')
      setForm({ nome: '', email: '', senha: '', funcao: opcoes[0]?.value || 'agent' })
      onCreated()
      onClose()
    } catch (err) {
